...
public class Staffchat implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        if (args.length >= 1) {
            sendStaffchat(commandSender, String.join(" ", args));
            return true;
        }
        ...
        if (playerData.staffchat()) {
            ...
            MessageManager.send(commandSender, Color.coloredMessage(PluginConfig.messagesStaffchatEnable, List.of("%prefix%"), List.of(PluginConfig.staffchatPrefix)));
            DatabaseManager.setStaffchat(player, true);
        }
        return true;
    }

    public static void sendStaffchat(CommandSender sender, String msg) {
        String permission = "chatsystem.command.staffchat";
        ...
        String staffchatPrefix = PluginConfig.staffchatPrefix + " ";

        if (sender instanceof Player player) {
            if (PluginConfig.staffchatEnableCustom) {
                    NameTag = Color.colored(PlaceholderAPI.setPlaceholders(player, PluginConfig.staffchatCustom));
            } else {
